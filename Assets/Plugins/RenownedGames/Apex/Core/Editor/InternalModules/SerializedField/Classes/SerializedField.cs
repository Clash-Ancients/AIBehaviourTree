...
namespace RenownedGames.ApexEditor
{
    public class SerializedField : SerializedMember, ISerializedField, IContainer
    {
        ...
        public SerializedField(SerializedObject serializedObject, string memberName) : base(serializedObject, memberName)
        {
            try
            {
        ...
        /// </summary>
        /// <param name="position">Rectangle position to draw field.</param>
        protected void OnUnityGUI(Rect position)
        {
            EditorGUI.PropertyField(position, serializedProperty, GetLabel(), true);
        }
        ...
        public void ApplyChildren()
        {
            if (serializedProperty.hasVisibleChildren)
            {
                ...
                if (value != null)
                {
                    ...
                    Type limitDescendant = value is MonoBehaviour ? typeof(MonoBehaviour) : typeof(Object);
                    foreach (MethodInfo methodInfo in type.AllMethods(limitDescendant))
                    {
#pragma warning disable CS0618 // Type or member is obsolete
                        SerializeMethodAttribute smAttribute = methodInfo.GetCustomAttribute<SerializeMethodAttribute>();
                        if (smAttribute != null)
                        {
                            try
                            {
                                SerializedMethod serializedMethod = new SerializedMethod(GetSerializedObject(), $"{serializedProperty.propertyPath}.{methodInfo.Name}")
                                {
                                    Repaint = Repaint
                                };
                            ...
                            {
                                Debug.LogError($"Failed to create a serialized method {methodInfo.Name} of the {GetSerializedObject().targetObject.GetType().Name} object. (Method has been ignored)");
                            }
                        }
#pragma warning restore CS0618 // Type or member is obsolete
                        ...
                        if (methodButtonAttribute != null)
                        {
                            if (methodButtonAttribute is ButtonAttribute)
                            {
                                try
                                {
                                    MethodButton button = new Button(GetSerializedObject(), $"{serializedProperty.propertyPath}.{methodInfo.Name}")
                                    {
                                        Repaint = Repaint
                                    };
                                ...
                                {
                                    Debug.LogError($"Failed to create a method button <b>{methodInfo.Name}</b> of the {GetSerializedObject().targetObject.GetType().Name} object. (Button has been ignored)\n<b><color=red>Exception: {ex.Message}</color></b>\n\nStacktrace:");
                                }
                            }
                            else if (methodButtonAttribute is RepeatButtonAttribute)
                            {
                                try
                                {
                                    MethodButton button = new RepeatButton(GetSerializedObject(), $"{serializedProperty.propertyPath}.{methodInfo.Name}")
                                    {
                                        Repaint = Repaint
                                    };
                            ...
                            else if (methodButtonAttribute is ToggleButtonAttribute)
                            {
                                try
                                {
                                    MethodButton button = new ToggleButton(GetSerializedObject(), $"{serializedProperty.propertyPath}.{methodInfo.Name}")
                                    {
                                        Repaint = Repaint
                                    };
        ...
        /// </summary>
        public void ClearInlineDecorators()
        {
            inlineDecorators = InlineDecoratorsNone;
        }
        ...
        /// </summary>
        public void ToggleOnLabelClick(bool value)
        {
            toggleOnLabelClick = value;
        }
        ...
        /// </summary>
        protected void RequestToUpdateParent()
        {
            UpdateParent?.Invoke();
        }
        ...
        private void RegisterCallbacks()
        {
            ...
            if (valueAttribute != null)
            {
                ...
                Type limitDescendant = GetDeclaringObject() is MonoBehaviour ? typeof(MonoBehaviour) : typeof(Object);
                foreach (MethodInfo methodInfo in type.AllMethods(limitDescendant))
                {
        ...
        public object GetManagedReference()
        {
        ...
        public void SetManagedReference(Type type)
        {
            SetManagedReference(Activator.CreateInstance(type));
        }

        public void SetManagedReference<T>()
        {
            SetManagedReference(typeof(T));
        }
        ...
        public string GetEnumDisplayValue(int valueIndex)
        {
            return serializedProperty.enumDisplayNames[valueIndex];
        }
        ...
        public string GetEnumValue(int valueIndex)
        {
            return serializedProperty.enumNames[valueIndex];
        }
        ...
        {
            return serializedProperty.enumDisplayNames.Length;
        }
        ...
        {
            return LayerMask.LayerToName(serializedProperty.intValue);
        }
        ...
        public T GetChild<T>(int index) where T : SerializedMember
        {
        ...
        public T FindRelative<T>(string path) where T : SerializedMember
        {
            return FindRelative(path) as T;
        }
