...
namespace RenownedGames.ApexEditor
{
    ...
    public sealed class ApexSettings : ScriptableSingleton<ApexSettings>
    {
        ...
        [SerializeField]
        [Array(OnGUI = nameof(OnExceptTypeGUI), GetHeight = nameof(GetExceptTypeHeight))]
        ...
        private void ApplySettings()
        {
            ...
            if(exceptTypes != null)
            {
                for (int i = 0; i < exceptTypes.Length; i++)
                {
                    ...
                    {
                        Debug.LogWarning($"<b>Apex Message:</b> An attempt to add two identical types for an exception. Remove duplicate types: <color=red>{exceptType.GetName()}</color>");
                    }
                }
            }
        }
        ...
        private float GetExceptTypeHeight(SerializedProperty array, int index)
        {
        ...
        public ExceptType[] GetExceptTypes()
        {
            return exceptTypes;
        }

        public void SetExceptTypes(ExceptType[] exceptTypes)
        {
