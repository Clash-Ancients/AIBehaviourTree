    ...
    public class ReorderableArray
    {
        ...
        public ReorderableArray(SerializedField serializedField)
        {
            ...
            onHeaderGUI = (position) =>
            {
                ...
                {
                    position.x += 4;
                    ApexStyles.BoldFoldout.Draw(position, headerName, false, false, serializedField.IsExpanded(), false);
                    position.x -= 4;
                }
            };

            onElementGUI = (position, index, isActive, isFocused) =>
            {
                serializedField.GetArrayElement(index).OnGUI(position);
            };
            ...
            onRemoveClick = (position, index) =>
            {
                serializedField.RemoveArrayElement(index);
            };

            reorderableList = new ReorderableList(serializedField.GetSerializedObject(), serializedField.GetSerializedProperty(), false, false, false, false)
            {
                headerHeight = 0,
                footerHeight = 0,
                ...
                {
                    return getElementHeight.Invoke(index) + VERTICAL_SPACE;
                },
                ...
                drawElementBackgroundCallback = (position, index, isActive, isFocused) =>
                {
                    ...
                    if (Event.current.type == EventType.Repaint)
                    {
                        if ((index + 1) % 2 == 0)
                        {
                            ApexStyles.BoxEntryEven.Draw(position, false, isActive, isActive, isFocused);
                        }
                        else
                        {
                            ApexStyles.BoxEntryOdd.Draw(position, false, isActive, isActive, isFocused);
                        }
                    }
                },
        ...
        /// Reorderable array constructor.
        /// </summary>
        public ReorderableArray(SerializedField serializedField, bool draggable) : this(serializedField)
        {
        ...
        {
            Draw(ApexGUILayout.GetControlRect(GetHeight()));
        }
        ...
        public bool GetShowCount()
        {
            return showCount;
        }

        public void SetShowCount(bool value)
        {
            showCount = value;
        }
